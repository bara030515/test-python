...
inHisc = open("test.txt","r", encoding="UTF-8")

index = 0
timer = 0
score = 0
hisc = inHisc.readline()
difficulty = 0
tsugi = 0
timerCount = 0
...
# 블럭 정보 저장
blockCount = [0,0,0,0,0,0] # 0~5까지 인덱스

def draw_neko():
    cvs.delete("NEKO") # 캔버스에서 네코를 삭제
...
def check_neko():
    ...
    for y in range(9):
        for x in range(7):
            if check[y][x] > 0:
                if check[y][x] == check[y][x + 1] and check[y][x] == check[y + 1][x] and check[y][x] == check[y + 1][x + 1]:
                    neko[y][x] = 7
                    neko[y][x + 1] = 7
                    neko[y + 1][x] = 7
                    neko[y + 1][x + 1] = 7
...
def drop_neko():
    ...
    for y in range(8, -1, -1):
        for x in range(8): # 모든 칸에 대해서 내 아래칸이 비어 있고 내 칸이 비어 있지 않으면 내 블록을 아래로 옮겨라
...
def set_neko():
    for x in range(8):
        neko[0][x] = random.randint(0, difficulty) # 불록울 생성 (0은 빈 블록 1~16은 블록)
...
def game_main():
    global index, timer, score, hisc, difficulty, tsugi, timerCount
    ...
    if index == 0:  # 타이틀 로고
        ...
        if 24 <= mouse_x and mouse_x < 24 + 72 * 8 and 24 <= mouse_y and mouse_y < 24 + 72 * 10:
            ...
            if mouse_c == 1:
                ...
                tsugi = 0
                index = 2
        cvs.delete("CURSOR") # ?? 이미지를 만들어 놓고 쓰지 않는 코드?
